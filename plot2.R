plot2 <- function() { 
  ...
  plotdata <- tapply(NEI$Emissions[baltimoredata],NEI$year[baltimoredata],FUN=sum)
  plot(as.numeric(names(plotdata)),plotdata,xaxt="n",xlim=c(1998,2008),
       xlab="Year",ylab="Total PM2.5 Emission (Tons)",main="Total PM2.5 Emission from all Sources in Baltimore City",type="b",pch=1)
